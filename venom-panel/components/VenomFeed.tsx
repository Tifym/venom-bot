          ...
          <tbody className="divide-y divide-white/5">
            {signals.map((sig, i) => (
              <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3 text-white/70">{sig.time}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded text-[10px] ${sig.dir === 'LONG' ? 'bg-venom/20 text-venom' : 'bg-alert/20 text-alert'}`}>
                    {sig.dir === 'LONG' ? '▲' : '▼'} {sig.dir}
                  </span>
                </td>
                <td className="px-4 py-3 text-toxic">{sig.score}</td>
                <td className="px-4 py-3 text-white/90">{sig.zone}</td>
                <td className="px-4 py-3 text-white/60">{sig.entry}</td>
                <td className="px-4 py-3">
                  {sig.isPending && <span className="text-toxic">PENDING</span>}
                ...
                <td className="px-4 py-3 flex gap-2">
                  ...
                  {sig.isPending && (
                    <>
                      <button className="p-1 hover:bg-white/10 rounded text-white/50 hover:text-venom"><Check size={14} /></button>
                      <button className="p-1 hover:bg-white/10 rounded text-white/50 hover:text-alert"><X size={14} /></button>
                    </>
                  )}
                </td>
              </tr>
